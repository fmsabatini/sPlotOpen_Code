...
usable.plots123 <- readr::read_csv("_output/header.sel.final.csv") %>% 
  ...
  mutate(Usable=ifelse(`GIVD ID` %in% (answers %>% 
                                        ...
                                        pull(GIVD.ID)), 
                       "Yes", Usable)) %>% 
  mutate(Usable=ifelse(`GIVD ID` %in% c("AF-00-008"), "No", Usable))  %>% #Hjalmar Kuhl retracted agreement  
  distinct()
...
header.oa %>% 
  ...
  mutate_at(.vars=vars(`Mosses identified (y/n)`, `Lichens identified (y/n)`),
         .funs=~forcats::fct_recode(., 
                                     ...
                                     "FALSE" = "N",
                                     "FALSE" = "n")) %>% 
  summarize(n.mosses=sum(`Mosses identified (y/n)`=="TRUE"), 
            n.lichens=sum(`Lichens identified (y/n)`=="TRUE"))
...
reference.oa <- bind_rows(br1.out, br1.out.123, br2.out, bib.db) %>% 
  distinct() %>% 
