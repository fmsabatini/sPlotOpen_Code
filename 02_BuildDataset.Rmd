...
Load PCA data
```{r}
load("_data/plot_data.RData") ##import pca data
pca.grids <- plot_data %>% 
...
whitt.biome <- whittaker_base_plot() +
  ...
  geom_point(data=climate.oa %>% 
             ...
             alpha=1/4, 
             cex=1/20) + 
    theme(plot.margin=margin(r=-0.35, unit="cm"))
```
...
biome.schu <- ggplot() + 
  ...
  geom_point(data=climate.oa %>% 
              ...
              filter(bio12<4500 & bio01>-11) %>% 
               mutate(Biome=factor(Biome, levels=biome.order))
  ...
  theme(axis.text.y = element_blank(),
        plot.margin=margin(l=-0.4, r=-0.1, unit="cm"))

```
Make a panel with the two plots
```{r}
panel.biomes <- cowplot::plot_grid(whitt.biome, biome.schu, nrow=1, align = "h", rel_widths = c(0.5, .5))
ggsave(filename="_output/Figure2.png", plot = panel.biomes, width = 11, height=4, units="in")
```
...
## Figure 3(?) - PCA graph + world map of selected plots

Create template of small world map
```{r}
...
load("/data/sPlot/users/Francesco/Global_Graphs/sPlot_PCA3.RData")
...
```{r, eval=f, echo=F}
## version 1, Create world rasters of PCA values and extract plot values by geographic intersection
...
### Project plots from sPlot Open in PCA space, based on geographic coordinates
header.shp <- SpatialPointsDataFrame(coords= header.oa %>% 
                                       dplyr::select(Longitude, Latitude),
                                     proj4string = CRS("+init=epsg:4326"), 
                                     data=data.frame(PlotObservationID= header.oa$PlotObservationID))

header.oa$PC1 <- raster::extract(PC1_r, header.shp)
header.oa$PC2 <- raster::extract(PC2_r, header.shp)
```


Assign pca values to `header.oa` and create data.frame for plotting, binding world pca and plot pca values
```{r}
header.oa <- header.oa %>% 
  left_join(pca.grids %>% 
              dplyr::select(PlotObservationID, PC1=pc1_val, PC2=pc2_val), 
            by="PlotObservationID")
