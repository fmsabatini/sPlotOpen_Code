...
load("_data/plot_data.RData") ##import plot-level pca data
...
## there are many plots without PCA value (e.g., on little islands, or in coastal area not matching the grid in SOILGRIDS)
## assign nearest neighbor
plot_nodata <- plot_data[which(is.na(plot_data@data$pc1_val)),]
dim(plot_nodata) #plots to assign

## round 1 - 5000 meter buffer
library(parallel)
library(doParallel)
ncores <- 8
...
system.time({
pc_5000 <- foreach(i=seq_along(plot_nodata), .packages=c('raster'), .combine=rbind) %dopar% { 
  pc_5000 <- extract(stack(PC1_r, PC2_r), plot_nodata[i,], buffer=5000, fun=function(x){median(x, na.rm=T)})
}
})
"user  system elapsed 
 37.454  12.041 305.120 "
## rearrange output and extract unassigned plots
plot_data@data <- plot_data@data %>% 
  left_join(plot_nodata@data %>% 
              dplyr::select(PlotObservationID) %>% 
              bind_cols(pc_5000 %>% 
                          as_tibble()), 
            by="PlotObservationID") %>% 
  mutate(pc1_val=coalesce(pc1_val, layer.1)) %>% 
  mutate(pc2_val=coalesce(pc2_val, layer.2)) %>% 
  dplyr::select(-layer.1, -layer.2)
plot_nodata <- plot_data[which(is.na(plot_data@data$pc1_val)),]
dim(plot_nodata) #9810 plots still to assign

## round 2 - 20000 meter buffer
system.time({
pc_20000 <- foreach(i=seq_along(plot_nodata), .packages=c('raster'), .combine=rbind) %dopar% { 
  pc_20000 <- extract(stack(PC1_r, PC2_r), plot_nodata[i,], buffer=20000, fun=function(x){median(x, na.rm=T)})
}
})
"user  system elapsed 
  9.931   4.970  78.776"
...
plot_data@data <- plot_data@data %>% 
  ...
  dplyr::select(-layer.1, -layer.2)
plot_nodata <- plot_data[which(is.na(plot_data@data$pc1_val)),]
dim(plot_nodata) #2591 plots remain unassigned

## round 3 - 50000 meter buffer
system.time({
pc_50000 <- foreach(i=seq_along(plot_nodata), .packages=c('raster'), .combine=rbind) %dopar% { 
  pc_50000 <- extract(stack(PC1_r, PC2_r), plot_nodata[i,], buffer=50000, fun=function(x){median(x, na.rm=T)})
}
})
...
plot_data@data <- plot_data@data %>% 
  ...
  dplyr::select(-layer.1, -layer.2)
plot_nodata <- plot_data[which(is.na(plot_data@data$pc1_val)),]
dim(plot_nodata) #394 plots remain unassigned
...
sPlot_reco <- rasterize(header.oa[, c("SoilClim_PC1", "SoilClim_PC2")], reco, fun="count")
...
There are 302 entries without continent info
```{r}
header.oa <- header.oa %>% 
...
reference.oa <- bind_rows(br1.out, br2.out, bib.db) %>% 
  distinct()
