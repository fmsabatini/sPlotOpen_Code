...
write_lines(br1, "_data/PlotLevelInfo/BiblioReference.clean.txt")
```
...
```{ruby, eval=F, engine.path = '~/.rubies/ruby-2.7.2/bin/ruby'}
puts RUBY_VERSION
```

```{ruby, eval =F, engine.path = '~/.rubies/ruby-2.7.2/bin/ruby'}
require "anystyle"
File.open("BiblioReference.clean.txt", "r") do |file_handle|
    file_handle.each_line do |ref|
      File.open("output.txt", mode:"a") {|f| f.write AnyStyle.parse ref }
    end
  end
```
...
# Save Ouput
```{r}
save(DT2.oa, header.oa, metadata.oa, CWM.oa, file = "_output/sPlot_OPEN.RData")
