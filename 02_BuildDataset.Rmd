...
The location of some RAINFOR plots is sensitive. I reduce the precision of the spatial coordinates
```{r}
header.oa <- header.oa %>% 
...
### Function to ceate reference list
...
table2 <- header.oa %>% 
  summarize_at(.vars=vars(!starts_with("PlotObservationID")),
            .funs = list(xxxNo.records=~sum(!is.na(.)),
               xxxType.of.variable=~ifelse(class(.)=="logical",
                                           "b",
                                           ifelse(class(.) %in% c("character", "factor"), 
                                                  "n",
                                                  ifelse(class(.)=="Date",
                                                         "d",
                                                         "q"))),
  ...
  dplyr::select(Variable, `Range/Levels`, `Unit of Measurement`=Unit.of.Measurement, `Nr. Records`=No.records, `Type`=Type.of.variable)
...
readr::write_lines("\n \n \n", file = out.file, append=T)
readr::write_lines("Table: Description of the variables contained in the ‘header’ matrix, together with their range (if numeric) or possible levels (if nominal or binary). Variable types can be n - nominal (i.e. qualitative variable), q - quantitative, or b - binary (i.e., boolean), or d - date. {#tbl:Table2 tag='2'}\n", file = out.file, append=T)
